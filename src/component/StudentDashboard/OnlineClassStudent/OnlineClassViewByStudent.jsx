...
import { useGetOnlineClassesQuery, useGetOnlineClassesStudentQuery } from '../../Services/OnlineClasses/OnlineClassesApi';
...
const OnlineClassViewByStudent = () => {
  const [searchQuery, setSearchQuery] = useState('');
  const userResponse = useSelector(state => state.idSlice.userResponse);
  const { data, isLoading, error ,refetch} = useGetOnlineClassesStudentQuery(userResponse._id)
  ...
  if (error) {
    return (
      <div className="p-6 text-center text-red-600">
        ...
        <button 
          onClick={refetch}
          className="mt-4 bg-custom-primary text-white px-4 py-2 rounded"
        >
          Retry
        </button>
      </div>
  ...
  return (
    <div className="container px-4 py-5 w-full">
      <div className=" p-6">
        <IconWithTitle
        ...
        <div className="w-full mb-6">
          <div className="flex border-b border-gray-200">
            ...
            >
              ...
              Upcoming
              <span className={`ml-2 px-2 py-0.5 text-xs rounded-full ${tabValue === 0 ? 'bg-custom-primary text-white' : 'bg-gray-200 text-gray-700'}`}>
...
export default OnlineClassViewByStudent